...
import com.br.aparecido.DateManager;
import com.br.aparecido.DateManagerException;
import com.br.aparecido.MonthlyDateManager;
import com.br.aparecido.WeeklyDateManager;
import com.br.aparecido.YearlyDateManager;

class DateManagerTest {

	@Test
	public void testInstantiatingDateManager() {
		...
		assertEquals(dateManager.getMinDateSelectable().getTime(), minSelectableDate.getTime());
	}

	@Test
	public void testWeeklyDataManager() {
		DateManager dateManager = new WeeklyDateManager();
		...
		try {
			...
			dateManager.setSelectedDate(minSelectableDate);
		...
		try {
			...
			assertEquals("Data Inválida " + minSelectableDate.getTime().toString(), e.getMessage());
		}
		...
		try {
			...
			assertEquals("Data Inválida " + today.getTime().toString(), e.getMessage());
		}
	}

	@Test
	public void testWeeklyGoToNextDate() {
		DateManager dateManager = new WeeklyDateManager();
		...
		try {
			dateManager.setSelectedDate(dayBeforeYesterday);
		} catch (DateManagerException e) {
			fail();
		}

		try {
			dateManager.goToNextDate();
			dateManager.goToNextDate();
		} catch (DateManagerException e) {
			fail();
		}
		...
		try {
			...
			assertEquals("Não é possível avançar para uma data além de hoje.", e.getMessage());
		}
	...
	@Test
	public void testWeeklyGoToPreviousDate() {
		DateManager dateManager = new WeeklyDateManager();

		assertTrue(dateManager.canGoToPreviousDate());

		try {
			dateManager.goToPreviousDate();
		} catch (DateManagerException e) {
			fail();
		}
		...
		try {
			...
			assertEquals("Não é possível regredir para uma data anterior a 01/01/1900.", e.getMessage());
		}
	}

	@Test
	public void testMonthlyDateManager() {
		DateManager dateManager = new MonthlyDateManager();
		...
		try {
			dateManager.setSelectedDate(finalDate);
			fail();

		} catch (DateManagerException e) {
			assertEquals("Data Inválida " + finalDate.getTime().toString(), e.getMessage());
		}

		startDate.add(Calendar.MONTH, -3);
		finalDate.add(Calendar.MONTH, -3);
		...
		try {
			...
			assertEquals("Data Inválida " + date.getTime().toString(), e.getMessage());
		}
	...
	@Test
	public void testMonthlyGoToNextDate() {
		DateManager dateManager = new MonthlyDateManager();
	...
	@Test
	public void testYearlyDateManager() {
		DateManager dateManager = new YearlyDateManager();
		...
		System.out.println(String.format("startDate = %d/%d/%d - %d:%d:%d",
				dateManager.getStartDate().get(Calendar.DAY_OF_MONTH), dateManager.getStartDate().get(Calendar.MONTH),
				dateManager.getStartDate().get(Calendar.YEAR), dateManager.getStartDate().get(Calendar.HOUR),
				dateManager.getStartDate().get(Calendar.MINUTE), dateManager.getStartDate().get(Calendar.SECOND)));

		System.out.println(String.format("finalDate = %d/%d/%d - %d:%d:%d",
				dateManager.getFinalDate().get(Calendar.DAY_OF_MONTH), dateManager.getFinalDate().get(Calendar.MONTH),
				dateManager.getFinalDate().get(Calendar.YEAR), dateManager.getFinalDate().get(Calendar.HOUR),
				dateManager.getFinalDate().get(Calendar.MINUTE), dateManager.getFinalDate().get(Calendar.SECOND)));
	}
}
